...
public class SubActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /* test *//*
        adapter.addItem(new P_AreaVO("A-1","A",0));
        adapter.addItem(new P_AreaVO("A-2","A",1));
        adapter.addItem(new P_AreaVO("A-3","A",0));
        adapter.addItem(new P_AreaVO("A-4","A",0));
        adapter.addItem(new P_AreaVO("A-5","A",1));
        adapter.addItem(new P_AreaVO("A-6","A",1));
        adapter.addItem(new P_AreaVO("A-7","A",1));
        adapter.addItem(new P_AreaVO("A-8","A",0));
        handler.post(new Runnable() {
            @Override
            public void run() {
                adapter.notifyDataSetChanged();
